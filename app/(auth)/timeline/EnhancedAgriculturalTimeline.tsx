...
import AddEditEntryModal, { type AddEditEntryModalHandle } from "./AddEditEntryModal"
...
function getAllSeasonsFromData(data: TimelineEntry[]): string[] {
  const allSeasons = (data && data.length > 0 && data.map((entry) => entry.muaVu || "").filter((s) => s !== "")) || []
...
const EnhancedAgriculturalTimeline: React.FC<EnhancedAgriculturalTimelineProps> = ({
    ...
    const newFiltered = data.filter((item) => {
      ...
      if (dateRange.from || dateRange.to) {
        if (item.ngayThucHien) {
          ...
          const itemDate = parse(item.ngayThucHien, "dd-MM-yyyy", new Date(), { locale: vi })
  ...
  const handleEditEntry = (updatedEntry: TimelineEntry) => {
    setData((prev) => prev.map((item) => (item._id === updatedEntry._id ? updatedEntry : item)))
  }
  ...
  if (filteredData.length === 0) {
    return (
      <EmptyState
        ...
        description="Không tìm thấy dữ liệu nhật ký phù hợp với bộ lọc hiện tại."
        action={{
          label: "Xóa bộ lọc",
          onClick: () => {
            setSeasonFilter("")
    ...
    <div className="max-w-3xl mx-auto flow-root">
      <ul role="list" className="-mb-8">
        {filteredData.map((entry, index) => (
          <li key={entry._id}>
            <TimelineEntryComponent data={entry} isLast={index === filteredData.length - 1} onEdit={openEditModal} />
          </li>
        ))}
      </ul>
