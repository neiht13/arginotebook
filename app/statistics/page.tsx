...
function makeMonthData(logs: typeof farmingLogs) {
...
function makePieData(logs: typeof farmingLogs) {
...
function makeRadarData(logs: typeof farmingLogs) {
...
export default function StatisticsPage() {
  ...
  const totalCost = filteredLogs.reduce(
    (acc, log) => acc + log.chiPhiVatTu + log.chiPhiCong,
    0
  )
  ...
  const exportToExcel = () => {
    try {
      ...
      const taskTypeData = Object.entries(taskTypes).map(([name, value]) => [
        name,
        value,
      ])
      const taskTypeSheet = XLSX.utils.aoa_to_sheet([
        ["Loại công việc", "Số lượng"],
        ...taskTypeData,
      ])
      XLSX.utils.book_append_sheet(workbook, taskTypeSheet, "Công việc theo loại")

      // Sheet: Công việc theo ngày
      const taskDateData = filteredLogs.reduce(
